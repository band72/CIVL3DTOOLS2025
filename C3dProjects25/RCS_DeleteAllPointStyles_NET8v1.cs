...
namespace RCS.C3D2025
{
    ...
    public class RcsDeleteAllPointStylesNet8
    {
                ...
                using (var tr = db.TransactionManager.StartTransaction())
                {
                    ...
                    foreach (var id in ids)
                    {
                        ...
                        catch (Autodesk.AutoCAD.Runtime.Exception aex)
                        {
                            ...
                            if (aex.ErrorStatus == Autodesk.AutoCAD.Runtime.ErrorStatus.CannotBeErasedByCaller ||
                                aex.ErrorStatus == Autodesk.AutoCAD.Runtime.ErrorStatus.ObjectIsReferenced ||
        ...
        {
            // Prefer Civil 3D StyleBase.Name when available so "Standard" is correctly detected.
            try
            {
                if (o is Autodesk.Civil.DatabaseServices.Styles.StyleBase sb)
                {
                    if (!string.IsNullOrWhiteSpace(sb.Name)) return sb.Name;
                }
            }
            catch { }
