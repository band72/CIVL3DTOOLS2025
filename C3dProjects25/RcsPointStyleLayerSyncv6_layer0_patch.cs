...
namespace RCS.C3D2025
{
    ...
    {
        [CommandMethod("RCS_SYNC_POINTSTYLE_LAYERS_FROM_DESC_KEYSv7")]
        public static void RCS_SYNC_POINTSTYLE_LAYERS_FROM_DESC_KEYS()
        {
            RcsError.RunCommandSafe("RCS_SYNC_POINTSTYLE_LAYERS_FROM_DESC_KEYSv7", () =>
            {
        ...
        [CommandMethod("RCS_DIAG_DESC_KEYSv1")]
        public static void RCS_DIAG_DESC_KEYS()
        {
            RcsError.RunCommandSafe("RCS_DIAG_DESC_KEYSv1", () =>
            {
                Document doc = Application.DocumentManager.MdiActiveDocument;
                if (doc == null) throw new InvalidOperationException("No active document.");
        ...
        private static Dictionary<string, string> BuildDescKeyNameToLayerMap(Transaction tr, Database db, Editor ed)
        {
            ...
            foreach (ObjectId setId in setIds)
            {
                ...
                foreach (ObjectId keyId in keyIds)
                {
                    ...
                    string layer = NormalizeLayerName(ResolveKeyLayerName(tr, key));
                    if (string.Equals(layer, "0", StringComparison.OrdinalIgnoreCase))
                    {
                        // Per requirement: any missing/unusable/none layer is forced to Layer 0
                        missingLayer++;
                    }
