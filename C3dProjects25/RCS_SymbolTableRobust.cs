...
{
    public class RCS_SymbolTableRobust
    {
        ...
        [CommandMethod("RCS_CreateSymbolTableRobust")]
        public void CreateSymbolTable()
        {
            ...
            try
            {
                ...
                {
                    // --- PRE-CALCULATION: Point Group Priority ---
                    // We need this to resolve points set to <default> style.
                    // GetDisplayOrder() returns the groups in Top-Down priority.
                    List<ObjectId> groupPriorityList = new List<ObjectId>();
                    foreach (ObjectId id in civilDoc.PointGroups)
                    {
                        groupPriorityList.Add(id);
                    }
                    ...
                    foreach (ObjectId pointId in cogoPointIds)
                    {
                        ...
                        {
                            // --- LOGIC CHANGE: Resolve the Active Style ---
                            ObjectId activeStyleId = point.StyleId;

                            // If style is Null (<default>), we must find the controlling Point Group
                            if (activeStyleId.IsNull)
                            {
                                // 1. Get all groups this point belongs to
                                var pointGroups = new List<ObjectId>();
                                foreach (ObjectId groupId in civilDoc.PointGroups)
                                {
                                    PointGroup pg = tr.GetObject(groupId, OpenMode.ForRead) as PointGroup;
                                    if (pg != null && pg.GetPointNumbers().Contains(point.PointNumber))
                                    {
                                        pointGroups.Add(groupId);
                                    }
                                }

                                // 2. Find which one is highest in the Display Order
                                foreach (ObjectId groupId in groupPriorityList)
                                {
                                    if (pointGroups.Contains(groupId))
                                    {
                                        // This is the controlling group
                                        PointGroup pg = tr.GetObject(groupId, OpenMode.ForRead) as PointGroup;
                                        activeStyleId = pg.PointStyleId;
                                        break; // Found the winner, stop searching
                                    }
                                }
                            }

                            // If we still don't have a valid style (rare), skip
                            if (activeStyleId.IsNull) continue;

                            PointStyle style = tr.GetObject(activeStyleId, OpenMode.ForRead) as PointStyle;
                            ObjectId symbolBlockId = ObjectId.Null;

                            // RULE: Check if style uses a Block (Symbol)
                    ...
                    {
                        ed.WriteMessage("\nError: No points with valid Block symbols found.");
                        return;
                    }
