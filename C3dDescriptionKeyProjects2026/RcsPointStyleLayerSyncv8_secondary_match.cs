...
namespace RCS.C3D2025
{
    ...
    /// </summary>
    public class RcsPointStyleLayerSyncv81
    {
        // --- Output files ---
        ...
        [CommandMethod("RCS_SYNC_POINTSTYLE_LAYERS_FROM_DESC_KEYSv81")]
        public static void RCS_SYNC_POINTSTYLE_LAYERS_FROM_DESC_KEYSv8()
        {
            RcsError.RunCommandSafe("RCS_SYNC_POINTSTYLE_LAYERS_FROM_DESC_KEYSv81", () =>
            {
        ...
        [CommandMethod("RCS_DIAG_DESC_KEYSv31")]
        public static void RCS_DIAG_DESC_KEYSv31()
        {
            ...
            out bool usedSecondary)
        {
            ...
            // 1) direct match
            if (descKeyNameToLayer.TryGetValue(pointStyleName, out keyLayer))
            {
                matchedKey = pointStyleName;
                return true;
            }

            // 2) secondary match by stripping a 2-char prefix
            if (pointStyleName.Length > 2)
            {
                string ps = pointStyleName.Trim();
                if (ps.Length > 2)
                {
