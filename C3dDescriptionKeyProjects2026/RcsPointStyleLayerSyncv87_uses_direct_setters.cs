...
namespace RCS.C3D2025
{
    ...
    public class RcsPointStyleLayerSyncv7
    {
        ...
        [CommandMethod("RCS_SYNC_POINTSTYLE_LAYERS_FROM_DESC_KEYSv87")]
        public static void RCS_SYNC_POINTSTYLE_LAYERS_FROM_DESC_KEYS()
        {
            RcsError.RunCommandSafe("RCS_SYNC_POINTSTYLE_LAYERS_FROM_DESC_KEYSv87", () =>
            {
                ...
                try
                {
                    ...
                    PromptKeywordOptions pko3 = new PromptKeywordOptions("\nSet Profile/Section display layers too? (may be unsupported)");
                    pko3.AllowNone = true;
...
[CommandMethod("RCS_DIAG_DESC_KEYS")]
        ...
        private static bool SetPointStyleDisplayLayerAllViews(
            ...
            bool setAllViews)
        {
            // Plan is REQUIRED; Profile/Section are optional because some Civil builds have incompatible signatures.
        ...
        // --- Robust display-style layer setter (direct property/method set) ---
            ...
            Editor ed)
        {
            ...
            {
                if (string.Equals(viewName, "Plan", StringComparison.OrdinalIgnoreCase))
                    RcsError.Warn(ed, "No display style found for Plan/" + componentHint + " on '" + GetStyleName(ps) + "'.");
                return false;
            }

            bool ok = TrySetLayerOnDisplayStyle(displayStyle, layerName, layerId);
            if (!ok && string.Equals(viewName, "Plan", StringComparison.OrdinalIgnoreCase))
                RcsError.Warn(ed, "No compatible layer setter for '" + GetStyleName(ps) + "' (Plan/" + componentHint + ").");
            return ok;
        }
        ...
        private static object GetDisplayStyleObject(object pointStyle, string viewName, string componentHint)
        {
            ...
            // 1) Property-based retrieval (no-arg; avoids signature mismatches)
            try
            {
                ...
                foreach (var pi in props)
                {
                    ...
                    bool viewMatch = string.IsNullOrWhiteSpace(v) || pn.IndexOf(v, StringComparison.OrdinalIgnoreCase) >= 0;
                    bool compMatch = string.IsNullOrWhiteSpace(c) || pn.IndexOf(c, StringComparison.OrdinalIgnoreCase) >= 0;
            ...
            // 2) Method-based retrieval (try multiple overloads)
            try
            {
                Type t = pointStyle.GetType();
                ...
                foreach (var mi in methods)
                {
                    ...
                    try
                    {
                        if (ps.Length == 1)
                        {
                            ...
                            var res = mi.Invoke(pointStyle, new object[] { a0 });
                            if (res != null) return res;
                        }
                        else if (ps.Length == 2)
                        {
                            object a0, a1;
                            if (!TryBuildArg(ps[0].ParameterType, viewName, out a0)) continue;
                            if (!TryBuildArg(ps[1].ParameterType, componentHint, out a1)) continue;
                            var res = mi.Invoke(pointStyle, new object[] { a0, a1 });
                            if (res != null) return res;
                        }
                    }
                    catch
        ...
        private static bool TryBuildArg(Type paramType, string hint, out object arg)
        {
            ...
            if (paramType != null && paramType.IsEnum)
            {
                ...
                {
                    foreach (string cand in new[] { h, h + "View", "Model" })
                    {
