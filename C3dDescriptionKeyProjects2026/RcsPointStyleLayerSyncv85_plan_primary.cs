...
namespace RCS.C3D2025
{
    ...
    public class RcsPointStyleLayerSyncv7
    {
        ...
        [CommandMethod("RCS_SYNC_POINTSTYLE_LAYERS_FROM_DESC_KEYSv85")]
        public static void RCS_SYNC_POINTSTYLE_LAYERS_FROM_DESC_KEYS()
        {
            RcsError.RunCommandSafe("RCS_SYNC_POINTSTYLE_LAYERS_FROM_DESC_KEYSv85", () =>
            {
...
[CommandMethod("RCS_DIAG_DESC_KEYS")]
        ...
        {
            // IMPORTANT:
            // In some Civil 3D builds, Profile/Section display-style setters have different signatures than Plan,
            // causing reflection calls to throw "Parameter count mismatch".
            //
            // RCS rule: Plan view is primary; do NOT fail the whole update if Profile/Section can't be set.
            bool planOk = false;

            try
            {
                planOk = TrySetViewInternal(ps, "Plan", componentHint, layerName, layerId, ed);
            }
            catch (System.Exception ex)
            {
                try { RcsError.Warn(ed, "Get/SetDisplayStyle failed: PointStyle Plan/" + componentHint + " -> " + layerName + " :: " + ex.Message); } catch { }
                planOk = false;
            }

            // Best-effort attempts (non-fatal)
            try { TrySetViewInternal(ps, "Profile", componentHint, layerName, layerId, ed); }
            catch { /* non-fatal */ }

            try { TrySetViewInternal(ps, "Section", componentHint, layerName, layerId, ed); }
            catch { /* non-fatal */ }
